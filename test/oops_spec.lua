...
context('inspection features', function ()
  context('isobject', function ()
    ...
    test('a number/table/string/nil is not an object', function ()
      assert_false(isobject({ x = 5 }))
      assert_false(isobject(5))
      assert_false(isobject('666'))
      assert_false(isobject(nil))
    end)
  end)

  context('isclass', function ()
    test('a class is a class', function ()
      assert_true(isclass(class { x = 5 }))
    end)
    test('a number/table/string/nil is not a class', function ()
      assert_false(isclass({ x = 5 }))
      assert_false(isclass(5))
      assert_false(isclass('666'))
      assert_false(isclass(nil))
    end)
  end)

  context('isinstanceof', function ()
    local C = class { }
    local C2 = class(C) { }

    test('an object is instance of it\'s class', function ()
      assert_true(isinstanceof(C(), C))
    end)
    test('an object is instance of it\'s class\' parent', function ()
      assert_true(isinstanceof(C2(), C))
    end)
    test('a number/table/string/nil is not an instance of a class', function ()
      assert_false(isinstanceof(5, C))
      assert_false(isinstanceof({ x = 5 }, C))
      assert_false(isinstanceof('5', C))
      assert_false(isinstanceof(nil, C))
    end)
    test('an object is not an instance of not a class', function ()
      assert_false(isinstanceof(C(), 5))
      assert_false(isinstanceof(C(), '5'))
      assert_false(isinstanceof(C(), { x = 5 }))
      assert_false(isinstanceof(C(), nil))
    end)
  end)
